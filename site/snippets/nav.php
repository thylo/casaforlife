<?php foreach (site()->content()->get("mainMenu")->toPages() as $index => $item): ?>
    <li class="o-list-bare__item c-menu__item menu-item-<?= $index ?><?= (site()->page()->id() == $item->id()) ? " active" : "" ?>">
        <a href="<?= url($item) ?>" title="Go to <?= $item ?>" class="c-menu__link c-link c-link--invert">
            <?= $item->title() ?>
        </a>
    </li>
<?php endforeach; ?>
